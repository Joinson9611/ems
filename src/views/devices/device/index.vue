    ...
    <el-dialog :visible.sync="isDialogHandleChannelShow" :append-to-body="true" :close-on-click-modal="false" :title="isChannelEdit? '编辑通道' : '新建通道'" @close="closeChannelDialog">
      <el-form ref="formChannel" :model="channelInfo" :rules="ChannelInfoRules" label-width="100px">
        ...
        <el-form-item label="通道地址" class="dialog-form-item" prop="channel_num">
          <el-input v-model="channelInfo.channel_num" type="text" />
        </el-form-item>
...
import { getChannelOutInfoList, addChannelOut, updateChannelOut, deleteChannelOut } from '@/api/channelOut'
import { getChannelInInfoList, addChannelIn, updateChannelIn, deleteChannelIn } from '@/api/channelIn'
...
export default {
  ...
  methods: {
    ...
    getChannelInInfoList() {
      this.isChannelLoadingShow = true
      const params = {
        project_id: this.selected_project_id,
        device_id: this.paramsGetChannelList.device_id
      }
      getChannelInInfoList(params).then(res => {
        this.isChannelLoadingShow = false
        this.channelList = res.data.items
        this.channelTotal = res.data.total
      }).catch(() => {
    ...
    onSearch() {
      ...
      this.paramGetDeviceInfoList.floor_id = this.tempFilterOptions.floor_id !== '' ? this.tempFilterOptions.floor_id : undefined
    ...
    getDeviceTypeList() {
      getDeviceTypeList({ device_type_group_id: 0 }).then(res => {
    ...
    deleteChannel() {
      this.$confirm('确认删除选中通道吗？', '提示', {
        ...
        if (this.paramsGetChannelList.device_type_group_id === 1) {
          const requestDeleteParams = {
            project_id: this.selected_project_id,
            channel_id_list: this.channelselection.map(item => item.id)
          }
          deleteChannelOut(requestDeleteParams).then(() => {
            this.$message({
              type: 'success',
              message: '删除成功'
            })
            this.getChannelOutInfoList()
          }).catch(err => {
            console.log(err)
          })
        } else {
          ...
          deleteChannelIn(requestDeleteParams).then(() => {
            this.$message({
              type: 'success',
              message: '删除成功'
            })
            this.getChannelInInfoList()
          }).catch(err => {
            console.log(err)
          })
        }
    ...
    handleChannel({ device, label, id: device_id, device_type_group_id, device_type }) {
      ...
      if (device_type_group_id === 1) {
        this.getChannelOutInfoList(device_id)
      } else {
        this.getChannelInInfoList(device_id)
      }
    ...
    getDeviceInfoList() {
      const Params = {
        ...
        floor_id: this.paramGetDeviceInfoList.floor_id,
