    ...
    >
      <el-table-column label="报警类型" align="center" >
        <template slot-scope="scope">
          <span :style="{color:getAlarmColor(scope.row.alarm_type)}" ><i>{{ scope.row.alarm_type | alarm_state }}</i></span>
        </template>
      </el-table-column>
...
import { camerasRecord } from '@/api/cameras'
...
export default {
  ...
  methods: {
    ...
    getCameraInfoList() {
      ...
      camerasRecord(params).then(response => {
