          ...
          <div class="filter-container">
            ...
            >
              ...
              <el-table-column label="监测值" align="center" width="90px" >
                <template slot-scope="scope">
                  <span>{{ scope.row.water_value }}{{ scope.row.device_type_id=='101'?'m':'MPa' }}</span>
                </template>
              </el-table-column>
...
<script>
import { getDevicewatersList, getDevicewatersDetail } from '@/api/devicewaters'
...
export default {
  ...
  data() {
    return {
      ...
      alarmTypeOptions: [
        ...
        { alarm_type_id: 1, alarm_type: '过高' },
        { alarm_type_id: 2, alarm_type: '过低' }
      ],
      deviceInfoList: [],
      online_state: [],
      alarm_state: [],
  ...
  watch: {
    alarm_type_id: {
      handler(id) {
        this.tempFilterOptions.alarm_type_id = id
        this.paramsGetDevicewaters.alarm_type_id = id
  ...
  methods: {
    ...
    toPageDetail(id) {
      this.$router.push({ path: '/iot/waterDetail', query: { id }})
    },
    getFloorList(id) {
    ...
    getDevicewaters() {
      ...
      getDevicewatersList(this.paramsGetDevicewaters).then(res => {
    ...
    getDevicewatersDetail() {
      getDevicewatersDetail({ project_id: this.selected_project_id, device_type_id: this.device_type_id }).then(res => {
