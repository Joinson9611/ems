      ...
      <div class="list">
        ...
        >
          ...
          <el-table-column label="在线状态" align="center" width="76px" >
            <template slot-scope="scope">
              <span :style="{color: getAlarmColorscope.row.online_state === 1 ? '#67C23A' : '#F56C6C'}">{{ scope.row.online_state === 1 ? '在线' : '离线' }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toPageDetail(id) {
      this.$router.push({ path: '/devrecord/keypartDetail', query: { id, device_type_id: this.$route.meta.roles[0] }})
    },
    getDeviceCameras() {
      this.isLoading = true
