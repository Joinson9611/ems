      ...
      <div class="list">
        ...
        >
          ...
          <el-table-column label="防拆状态" align="center" width="76px" >
            <template slot-scope="scope">
              <span :style="{color:getAlarmColor(scope.row.tamper_state)}">{{ scope.row.tamper_state | tamper_state }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      alarmOptions: [{ id: 0, type: '正常' }, { id: 1, type: '入侵报警' }],
  ...
  methods: {
    ...
    toPageDetail(id) {
      this.$router.push({ path: '/devrecord/infdetDetail', query: { id, device_type_id: this.$route.meta.roles[0] }})
    },
    getDeviceCameras() {
      this.isLoading = true
